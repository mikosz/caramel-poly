...
// CRC implementation based on https://stackoverflow.com/a/9842857/1794022
...
constexpr uint32_t crc32Recursive(const char* str, int idx)
{
	if (idx == -1) {
	    return 0xffffffff;
	} else {
		const auto prev = crc32Recursive(str, idx - 1);
		return (prev >> 8) ^ crc_table[(prev ^ str[idx]) & 0x000000ff];
	}
}

constexpr uint32_t crc32(const char* str, size_t length) {
	if (length == 0) {
		throw std::out_of_range("Length must be greater than 0");
	}
	return crc32Recursive(str, static_cast<int>(length - 1)) ^ 0xffffffff;
}
...
public:
	...
	constexpr uint32_t crc32() const {
		return detail::crc32(chars_, length_);
	}
